function main_rrt_rs()
    ...
    max_iter = 10000; step_size = 0.5; goal_sample_rate = 0.4; radius = 10; R = 7;
    ...
    for iter = 1:max_iter
        ...
        if ~collision_check_line(nearest_node.pos, new_pos, obstacle_list, space)
            ...
            if norm(new_node.pos(1:2) - goal(1:2)) < 15
    ...
    if best_goal_node > 0
        ...
        rrt_smooth_path = path_clothoid_smoothing(rrt_path);
        
        rs_smooth_path = path_clothoid_smoothing(rs_to_goal);
        
        % 전체 경로 = RRT* + RS
        full_path = [rrt_smooth_path; rs_smooth_path];
        assignin('base', 'full_smooth_path', full_path);
        
        % === 시각화 ===
        start_pt = full_path(1,1:2);
        goal_pt  = full_path(end,1:2);
        plot(full_path(:,1), full_path(:,2), 'r-', 'LineWidth', 2.5);
        % plot(goal(1), goal(2), 'ro', 'MarkerSize', 8, 'LineWidth', 2);
        % plot(full_path(:,1), full_path(:,2), '-o','LineWidth',2,'MarkerSize',4,'Color','b');
...
function sample = informed_sample(start, goal, c_max, ~)
...
function smooth_path = path_clothoid_smoothing(path)
    ...
    density_factor = 10;    % 포인트 밀도 증가 계수 (기존보다 몇 배 늘릴지)
    ...
    ts = linspace(0, s(end), density_factor * numel(s));
...
function [start, goal, obstacle_list, space] = map_data()
    start = [5.6, -36.6, 0];
    goal  = [18, -10.5, -pi/2];
    space = [5.5, 47.5, -44.9, -4.1];
    car_w = 1.97; car_l = 4.47;
    r = car_w / 2 + 0.5;
    n_circles = 5;
    interval = car_l / n_circles;
    traffic_info = [7.29999999999999   -28.7000000000000   -1.57079628000000;   12.8000000000000   -6.80000000000000   -1.57079628000000;   21.3000000000000   -6.60000000000000   -1.57079628000000;   30   -6.50000000000000   -1.57079628000000;   41.7000000000000   -6.30000000000000   -1.57079628000000;   6.90000000000001   -6.90000000000000   -1.57079628000000;   7.00000000000000   -21.8000000000000   1.57079628000000;   10.0000000000000   -21.8000000000000   1.57079628000000;   24.3000000000000   -21.9000000000000   1.57079628000000;   36.0000000000000   -21.9000000000000   1.57079628000000;   38.8000000000000   -21.8000000000000   1.57079628000000;   41.8000000000000   -21.8000000000000   1.57079628000000;   12.6000000000000   -28.8000000000000   -1.57079628000000;   18   -28.9000000000000   -1.57079628000000;   41.6000000000000   -28.9000000000000   -1.57079628000000;   9.90000000000000   -44.4000000000000   1.57079628000000;   21.4000000000000   -44.3000000000000   1.57079628000000;   24.5000000000000   -44.4000000000000   1.57079628000000;   21   -22   1.57079628000000;   17.5000000000000   -22   1.57079628000000;   27.5000000000000   -22   1.57079628000000];
    % traffic_info = [
    %     10, -35, pi/2;
    %     45, -55, pi/2;
    %     65, -75, pi/2;
